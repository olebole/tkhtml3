 ...
 *---------------------------------------------------------------------------
 */
int inlineContextPushBorder(p, pBorder)
    InlineContext *p;
 ...
 *---------------------------------------------------------------------------
 */
InlineBorder *inlineContextGetBorder(pLayout, pNode, parentblock)
...
{
    if (force || (p->nInline > 0 && p->whiteSpace == CSS_CONST_PRE)){
        InlineBox *pBox;
    ...
    int iOffset;
{
    ...
    inlineContextSetBoxDimensions(
        pContext, iWidth, -1 * iOffset, iHeight + iOffset);
}
