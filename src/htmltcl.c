...
static struct HtmlSubcommand {
    ...
    "text", "find", 4, 7, "TEXT ?nocase? ?before|after INDEX?",
                HtmlTextFindCmd}, {
    ...
    "token", "onEvents", 3, 5,
                "?-begin INDEX? ?-end INDEX? ?-range {INDEX INDEX}?",
                HtmlTokenOnEvents}, {
...
int
HtmlCommandAdd(c1, c2, m, n, help, xFunc)
    char *c1;
    char *c2;
    int m;
    int n;
...
Tcl_CmdProc(*xFunc);
{
    ...
    for (i = 0, pCmd = aSubcommand; i < nSubcommand; i++, pCmd++) {
        if (!pCmd->zCmd1) {
            pCmd->zCmd1 = (c1 ? strdup(c1) : 0);
            pCmd->zCmd2 = (c2 ? strdup(c2) : 0);
            pCmd->minArgc = m;
            pCmd->maxArgc = n;
...
int
HtmlCommandAddObj(c1, c2, m, n, help, xFunc)
    char *c1;
    char *c2;
    int m;
    int n;
    ...
    char *c2;
{
    ...
    for (i = 0, pCmd = aSubcommand; i < nSubcommand; i++, pCmd++) {
        if (!pCmd->zCmd1)
            continue;
        if ((!strcmp(pCmd->zCmd1, c1))
            && ((!pCmd->zCmd2) || (!strcmp(pCmd->zCmd2, c2))))
...
int
HtmlWidgetObjCommand(clientData, interp, objc, objv)
    ...
    Tcl_Obj *CONST objv[];             /* Argument strings. */
{
    ...
    for (i = 0, pCmd = aSubcommand; i < nSubcommand; i++, pCmd++) {
        ...
        if (pCmd->zCmd2) {
            ...
            if (strncmp(pCmd->zCmd2, arg2, length2))
                continue;
        }
...
int
HtmlBase64decodeCmd(clientData, interp, objc, objv)
...
int
HtmlBase64encodeCmd(clientData, interp, objc, objv)
    ClientData clientData;
    Tcl_Interp *interp;
    int objc;
    Tcl_Obj *const *objv;
...
#define P(n,s) ((pos+n)>ilen?'=':ev[s])
        ...
        c[i++] = P(2,
                   ((ib[pos + 1] << 2) & 0x3c) | ((ib[pos + 2] >> 6) & 0x03));
    ...
    int len;
{
    ...
    if (flags & 0x10) {
        while (i < len && s[i] && (signed char) s[i] != EOF)
            i++;
        i++;
    }
    if (flags & 0x8) {
...
static int
HtmlGunzipCmd(clientData, interp, objc, objv)
    ClientData clientData;
    Tcl_Interp *interp;
    int objc;
    Tcl_Obj *const *objv;
{
    int r, ulen = 1024, blen = ulen, fnlen;
    uLongf l = 0;
    ...
    if (!strcmp(from, "file")) {
        ...
        zf.zfree = Z_NULL;
        zf.opaque = Z_NULL;
        if (Z_OK != inflateInit2(&zf, -MAX_WBITS))
            goto gunziperror;
        fn = uncompr;
        for (;;) {
            zf.next_out = fn;
            ...
            l = zf.total_out;
            if (zf.total_in == fnlen || zf.avail_in == 0)
                break;
            blen += 1024;
            uncompr = (char *) HtmlRealloc(uncompr, blen);
            fn = uncompr + l;
...
static int
HtmlGzipCmd(clientData, interp, objc, objv)
    ClientData clientData;
    Tcl_Interp *interp;
    int objc;
    Tcl_Obj *const *objv;
{
    int i, n, r, l = 0, ilen, fnlen;
    unsigned char *ip;
...
static int
HtmlCrc32Cmd(clientData, interp, objc, objv)
    ClientData clientData;
    Tcl_Interp *interp;
    int objc;
    Tcl_Obj *const *objv;
...
int
HtmlStdchanCmd(clientData, interp, objc, objv)
    ClientData clientData;
    Tcl_Interp *interp;
    int objc;
    Tcl_Obj *const *objv;
{
    ...
    if (!strcmp(stype, "stdin"))
        type = TCL_STDIN;
    else if (!strcmp(stype, "stdout"))
        type = TCL_STDOUT;
    else if (!strcmp(stype, "stderr"))
        type = TCL_STDERR;
    else
        return TCL_ERROR;
...
static int
textfmtcmd(clientData, interp, objc, objv)
    ClientData clientData;
    Tcl_Interp *interp;
    int objc;
    Tcl_Obj *const *objv;
...
static int
lockcopycmd(clientData, interp, objc, objv)
    ClientData clientData;
    Tcl_Interp *interp;
    int objc;
    Tcl_Obj *const *objv;
...
static int
xorstrcmd(clientData, interp, objc, objv)
    ClientData clientData;
    Tcl_Interp *interp;
    int objc;
    Tcl_Obj *const *objv;
{
    ...
    char *ib;
    unsigned char *ip;
    int i = 0, n = 0, p = -1, ilen, olen, random = 0, ibegin = 0, dorandom =
            0, doimagic = 0;
...
static int
decryptsrcsub(clientData, interp, objc, objv, decrypt)
    ClientData clientData;
    Tcl_Interp *interp;
    int objc;
    Tcl_Obj *const *objv;
...
static int
srcencryptcmd(clientData, interp, objc, objv)
    ClientData clientData;
    Tcl_Interp *interp;
    int objc;
    Tcl_Obj *const *objv;
...
static int
srcdecryptcmd(clientData, interp, objc, objv)
    ClientData clientData;
    Tcl_Interp *interp;
    int objc;
    Tcl_Obj *const *objv;
...
static int
HtmlExitCmd(clientData, interp, objc, objv)
    ClientData clientData;
    Tcl_Interp *interp;
    int objc;
    Tcl_Obj *const *objv;
...
static int
HtmlReformatCmd(clientData, interp, objc, objv)
    ClientData clientData;
    Tcl_Interp *interp;
    int objc;
    Tcl_Obj *const *objv;
{
    ...
    if (CmpStr(dtype, "plain")) {
        if (!(CmpStr(stype, "http") || CmpStr(stype, "url"))) {
            Tcl_AppendResult(interp,
                             "html reformat: source type must be \"http\" or \"url\": ",
                             dtype, 0);
            return TCL_ERROR;
        }
        for (i = 0; i < l; n++, i++) {
    ...
    if (!CmpStr(stype, "plain")) {
        Tcl_AppendResult(interp,
                         "html reformat: source type must be \"plain\": ",
                         dtype, 0);
        return TCL_ERROR;
    }
    if (CmpStr(dtype, "url") || CmpStr(dtype, "http")) {
        for (i = 0; i < l; n++, i++) {
            ...
            if (isalnum(ch) || ch == '$' || ch == '-' || ch == '_' ||
                ...
                buf[n++] = '%';
                sprintf(buf + n, "%02X", (unsigned char) ch);
                n++;
            }
        }
        buf[n] = 0;
  ...
  /*
   * html urlsplit $url ** ** Split a URL into a list of its parts. 
   */
static int
HtmlUrlsplitCmd(clientData, interp, objc, objv)
    ClientData clientData;             /* The HTML widget data structure */
    Tcl_Interp *interp;                /* Current interpreter. */
    int objc;                          /* Number of arguments. */
    Tcl_Obj *CONST objv[];             /* Argument strings. */
{
  ...
  /*
   * html urljoin $scheme $authority $path $query $fragment ** ** Merge
   * together the parts of a URL into a single value URL. 
   */
static int
HtmlUrljoinCmd(clientData, interp, objc, objv)
    ClientData clientData;             /* The HTML widget data structure */
    Tcl_Interp *interp;                /* Current interpreter. */
    int objc;                          /* Number of arguments. */
    Tcl_Obj *CONST objv[];             /* Argument strings. */
...
static struct HtmlCmd {
    ...
    char *zHelp;                       /* Help string if wrong number of
                                        * arguments */
    int (*xFuncObj) (ClientData, Tcl_Interp *, int, Tcl_Obj * CONST objv[]);    /* Obj 
                                                                                 * cmd 
                                                                                 */
} acommand[] = {
    {
    ...
    "xor", "xor", 5, 8, "DATA PASSWORD ?-imagic -omagic -randomize?",
                xorstrcmd}, {
...
int
HtmlCommandObj(clientData, interp, objc, objv)
    ClientData clientData;             /* The HTML widget data structure */
    Tcl_Interp *interp;                /* Current interpreter. */
    int objc;                          /* Number of arguments. */
    Tcl_Obj *CONST objv[];             /* Argument strings. */
