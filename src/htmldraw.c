static char const rcsid[] =
        "@(#) $Id: htmldraw.c,v 1.34 2005/03/23 01:36:54 danielk1977 Exp $";
...
*/
static void
UnlinkAndFreeBlock(htmlPtr, pBlock)
    HtmlWidget *htmlPtr;
    HtmlBlock *pBlock;
{
...
*/
static void
AppendBlock(htmlPtr, pToken, pBlock)
...
*/
static void
GetLetterIndex(zBuf, index, isUpper)
    char *zBuf;
    int index;
    int isUpper;
{
...
*/
static void
DrawSelectionBackground(htmlPtr, pBlock, drawable, x, y)
    HtmlWidget *htmlPtr;               /* The HTML widget */
    HtmlBlock *pBlock;                 /* The block whose background is drawn 
                                        */
    Drawable drawable;                 /* Draw the background on this
                                        * drawable */
    int x;                             /* X coord of top-left of drawable */
    int y;                             /* Y coord of top-left of drawable */
{
    int xLeft, xRight;                 /* Left and right bounds of box to
                                        * draw */
    int yTop, yBottom;                 /* Top and bottom of box */
    HtmlElement *p = 0;                /* First element of the block */
    Tk_Font font;                      /* Font */
    GC gc;                             /* GC for drawing */
    XRectangle xrec;                   /* Size of a filled rectangle to be
                                        * drawn */
    ...
    if (pBlock == htmlPtr->pSelStartBlock && htmlPtr->selStartIndex > 0) {
        if (htmlPtr->selStartIndex >= pBlock->n) {
            return;
        }
        p = pBlock->base.pNext;
...
*/
static void
HtmlDrawRect(htmlPtr, drawable, src, x, y, w, h, depth, relief)
    HtmlWidget *htmlPtr;               /* The HTML widget */
    Drawable drawable;                 /* Draw it here */
    HtmlElement *src;                  /* Element associated with drawing */
    int x;                             /* Coordinates of the rectangle */
    int y;
    int w;
    int h;
    int depth;                         /* Width of the relief, or the flat
                                        * line */
    int relief;                        /* The relief.  TK_RELIEF_FLAT omits
                                        * 3d */
{
    if (depth > 0) {
        int i;
        ...
        for (i = 0; i < depth && i < h / 2; i++) {
            XDrawLine(htmlPtr->display, drawable, gcLight, x + i, y + i,
                      x + w - i - 1, y + i);
...
*/
void
HtmlBlockDraw(htmlPtr, pBlock, drawable, drawableLeft, drawableTop,
              drawableWidth, drawableHeight, pixmap)
    ...
    int drawableLeft;                  /* Virtual coordinate of left edge of
                                        * drawable */
    int drawableTop;                   /* Virtual coordinate of top edge of
                                        * drawable */
    ...
    Pixmap pixmap;
{
    ...
    HtmlElement *src;                  /* HtmlElement holding style
                                        * information */
    HtmlElement *pTable;               /* The table (when drawing part of a
                                        * table) */
    ...
    if (pBlock->n > 0) {
        ...
        Tk_DrawChars(htmlPtr->display,
                     drawable,
                     gc, font,
                     pBlock->z, pBlock->n, x - drawableLeft, y - drawableTop);
        ...
        switch (src->base.type) {
            case Html_LI:
                ...
                gc = HtmlGetGC(htmlPtr, src->base.style.color,
                               src->base.style.font);
                switch (src->li.type) {
                    ...
                    case LI_TYPE_Bullet1:
                        XFillArc(htmlPtr->display,
                                 drawable,
                                 gc,
                                 x - 7 - drawableLeft, y - 8 - drawableTop, 7,
                                 7, 0, 360 * 64);
                        break;
            ...
            case Html_TD:{
                    ...
                    if (((!htmlPtr->bgimage) || src->base.style.expbg) &&
                        ...
                        HtmlDrawRect(htmlPtr, drawable, src,
                                     ...
                                     src->cell.w, src->cell.h, depth, relief);
                    }
                    /*
                    ...
                    if (src->cell.bgimage)
                        HtmlTblBGDraw(htmlPtr, src->cell.x, src->cell.y,
                                      src->cell.w, src->cell.h, pixmap,
                                      src->cell.bgimage);
                    else if (src->cell.pRow
                             && (bgimg = src->cell.pRow->ref.bgimage))
                        HtmlTblBGDraw(htmlPtr, src->cell.x, src->cell.y,
            ...
            case Html_IMG:
                if (src->image.pImage) {
                    HtmlDrawImage(htmlPtr, src, drawable, drawableLeft,
                                  drawableTop, drawableLeft + drawableWidth,
    ...
    int drawableBottom;                /* bottom edge of the drawable */
{
    ...
    if (!pElem->image.pImage->anims)
        Tk_RedrawImage(pElem->image.pImage->image, imageX, imageY, imageW,
                       imageH, drawable, x, y);
    else {
        int redraw = htmlPtr->flags & ANIMATE_IMAGES;
        int i;
        HtmlImageAnim *a;
        HtmlImage *zImg = pElem->image.pImage;
        if ((!zImg->cur) || redraw)
            Tk_RedrawImage(pElem->image.pImage->image, imageX, imageY, imageW,
                           imageH, drawable, x, y);
    ...
    HtmlBlock *p;
{
    ...
    int i;
    HtmlStyle style;
    int firstSelected;                 /* First selected character in this
                                        * block */
    int lastSelected;                  /* Last selected character in this
                                        * block */
    char zBuf[2000];
...
/*            go = 0;  This cause a hang, instead lets do what we can. */
                        ...
                        strncpy(&zBuf[n], pElem->text.zText,
                                sizeof(zBuf) - n - 1);
            ...
            default:
                if (pElem->base.flags & HTML_Visible)
                    go = 0;
                break;
        }
        if (go == 0)
...
*/
static HtmlElement *
FindStartOfNextBlock(htmlPtr, p, pCnt)
    HtmlWidget *htmlPtr;
    HtmlElement *p;
    int *pCnt;
{
