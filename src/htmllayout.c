...
*/
static void
HtmlPopMargin(ppMargin, tag, pLC)
...
*/
static void
PopExpiredMargins(ppMarginStack, y)
    HtmlMargin **ppMarginStack;
    int y;
{
    ...
    for (; p && p != pEnd; p = p ? p->pNext : 0) {
        ...
        switch (p->base.type) {
            ...
            case Html_IMG:
                ...
                TRACE(HtmlTrace_GetLine2,
                      ("Place in-line image %s at x=%d w=%d\n",
...
*/
static int
FixLine(pStart, pEnd, bottom, width, actualWidth, leftMargin, maxX)
    HtmlElement *pStart;               /* Start of tokens for this line */
    HtmlElement *pEnd;                 /* First token past end of this line.
                                        * Maybe NULL */
    int bottom;                        /* Put the top of this line here */
    int width;                         /* This is the space available to the
                                        * line */
    int actualWidth;                   /* This is the actual width needed by
                                        * the line */
    int leftMargin;                    /* The current left margin */
    int *maxX;                         /* Write maximum X coordinate of ink
                                        * here */
{
    int dx;                            /* Amount by which to increase all X
                                        * coordinates */
    int maxAscent;                     /* Maximum height above baseline */
    int maxTextAscent;                 /* Maximum height above baseline for
                                        * text */
    int maxDescent;                    /* Maximum depth below baseline */
    int ascent, descent;               /* Computed ascent and descent for one 
                                        * element */
    HtmlElement *p;                    /* For looping */
    int y;                             /* Y coordinate of the baseline */
    int dy2center;                     /* Distance from baseline to text font 
                                        * center */
    int max = 0;

    if (actualWidth > 0) {
        for (p = pStart; p && p != pEnd && p->base.type != Html_Text;
             p = p->pNext) {
        }
        if (p == pEnd || p == 0)
            p = pStart;
        ...
        for (p = pStart; p && p != pEnd; p = p->pNext) {
            ...
            switch (p->base.type) {
                ...
                case Html_APPLET:
                    ...
                    dy2center =
                            (p->input.textDescent - p->input.textAscent) / 2;
    ...
    HtmlElement *p;
{
    ...
    switch (p->base.type) {
        ...
        case Html_BLOCKQUOTE:
            HtmlPushMargin(&pLC->leftMargin, HTML_INDENT, -1,
                           Html_EndBLOCKQUOTE);
            HtmlPushMargin(&pLC->rightMargin, HTML_INDENT, -1,
                           Html_EndBLOCKQUOTE);
            Paragraph(pLC, p);
            break;
        case Html_EndBLOCKQUOTE:
        ...
        case Html_IMG:
            switch (p->image.align) {
                case IMAGE_ALIGN_Left:
                    ...
                    HtmlPushMargin(&pLC->leftMargin, p->image.w + 2,
                                   y + p->image.h, 0);
                ...
                case IMAGE_ALIGN_Right:
                    ...
                    p->image.ascent = 0;
                    p->image.descent = p->image.h;
                    HtmlPushMargin(&pLC->rightMargin, p->image.w + 2,
                                   y + p->image.h, 0);
                    SETMAX(pLC->maxY, y + p->image.h);
                    SETMAX(pLC->maxX, x + p->image.w);
                    break;
        ...
        case Html_PRE:
            ...
            while (pNext->base.type == Html_Space) {
                ...
                pNext = pNext->pNext;
                if (pThis->base.flags & HTML_NewLine) {
                    break;
                }
            }
            Paragraph(pLC, p);
        ...
        case Html_BR:
            ...
            /*
             * All of the following tags need to be handed to the GetLine()
             * routine 
             */
        case Html_Text:
        case Html_Space:
    ...
    HtmlLayoutContext *pLC;
{
    ...
    for (p = pLC->pStart; p && p != pLC->pEnd; p = pNext) {
        ...
        while (1) {
             ...
             */
            pNext = GetLine(pLC, p, pLC->pEnd, lineWidth,
                            pLC->left - leftMargin, &actualWidth);
            TRACE(HtmlTrace_GetLine,
                  ("GetLine page=%d left=%d right=%d available=%d used=%d\n",
                   pLC->pageWidth, pLC->left, pLC->right, lineWidth,
                   actualWidth));
    ...
    HtmlWidget *htmlPtr;
{
    ...
    if (htmlPtr->marginwidth) {
        HtmlPushMargin(&pLC->leftMargin, htmlPtr->marginwidth, -1,
                       Html_EndBLOCKQUOTE);
        HtmlPushMargin(&pLC->rightMargin, htmlPtr->marginwidth, -1,
                       Html_EndBLOCKQUOTE);
    }
}
    ...
    HtmlWidget *htmlPtr;
{
    ...
    if (pLC->pStart) {
        ...
        if (htmlPtr->zGoto
            && ((p = HtmlAttrElem(htmlPtr, "name", htmlPtr->zGoto + 1)))) {
