static char const rcsid[] =
        "@(#) $Id: htmltable.c,v 1.52 2005/03/23 01:36:54 danielk1977 Exp $";
...
*/
int
HtmlTableText(htmlPtr, p, interp, flags, str)
    HtmlWidget *htmlPtr;
    HtmlElement *p;
    Tcl_Interp *interp;
    ...
    Tcl_DString *str;
{
    int j, Nest = 0, intext = 0, rows = 0, cols = 0, numcols = 0, h, w, maxh =
            1;
    ...
    Tcl_DString substr;                /* Temp to collect current cell
                                        * string. */
    ...
    while (p && (p = p->pNext)) {
        ...
        switch (p->base.type) {
            ...
            case Html_TD:
            case Html_TH:
                if ((!(cp = HtmlMarkupArg(p, "colspan", 0)))
                    || (cspans = atoi(cp)) <= 0)
                    cspans = 1;
...
*/
static HtmlElement *
TableDimensions(htmlPtr, pStart, lineWidth)
                                        ...
                                        */
{
    ...
    int minW, maxW, requestedW;        /* min, max, requested width for a
                                        * cell */
    ...
    int margin;                        /* Space between left margin and 1st
                                        * col */
    int availWidth;                    /* Part of lineWidth still available */
    int maxTableWidth;                 /* Amount of lineWidth available to
                                        * table */
    int fromAbove[HTML_MAX_COLUMNS + 1];        /* Cell above extends thru
                                                 * this row */
     ...
     * ** A+1 and B+1.  This information is used to add in the constraints
     * imposed ** by <TD COLSPAN=N> markup where N>=2. 
     */
...
# define ColReq(A,B) colMin[(B)-1][(A)-1]
    ...
    if (pStart->table.bgimage)
        pStart->table.hasbg = 1;
...
#ifdef DEBUG
    /*
     * The HtmlTrace_Table4 flag causes tables to be draw with borders ** of
     * 2, cellPadding of 5 and cell spacing of 2.  This makes the ** table
     * clearly visible.  Useful for debugging. 
     */
    ...
    for (p = pStart->pNext; p; p = pNext) {
        ...
        switch (p->base.type) {
            ...
            case Html_TH:
                ...
                hasbg = (pStart->table.hasbg || p->cell.pRow->ref.bgimage
                         || p->cell.bgimage);
                ...
                TRACE(HtmlTrace_Table1,
                      ("Row %d Column %d: min=%d max=%d req=%d stop at %s\n",
                       ...
                       HtmlTokenName(htmlPtr, p->cell.pEnd)));
                if (noWrap) {
                ...
                if (rowspan > 1) {
                    for (i = iCol;
                         i < iCol + p->cell.colspan && i < HTML_MAX_COLUMNS;
                         i++) {
    ...
    for (i = 1; i <= pStart->table.nCol; i++) {
        ...
        /*
         * Expand the minW[] of columns to accomodate "colspan=N"
         * constraints. ** The minW[] is expanded up to the maxW[] first.
         * Then all the maxW[]s ** are expanded in proportion to their sizes. 
         * The same thing occurs ** for reqW[]s. 
         */
        sumReq = reqW[i];
        ...
        for (j = i - 1; j >= 1; j--) {
            ...
            if (cmin > sumMin) {
                ...
                if (sumMin < sumMax) {
                    scale = (double) (cmin - sumMin) / (double) (sumMax -
                                                                 sumMin);
                    for (k = j; k <= i; k++) {
                        sumMin -= tminW[k];
            ...
            if (creq > sumReq) {
                ...
                int *tmaxW = pStart->table.maxW;
                if (sumReq < sumMax) {
                    scale = (double) (creq - sumReq) / (double) (sumMax -
                                                                 sumReq);
                    for (k = j; k <= i; k++) {
                        sumReq -= reqW[k];
    ...
    if (requestedW > pStart->table.minW[0]) {
        ...
        if (tmaxW[0] > tminW[0]) {
            scale = (double) (requestedW - tminW[0]) / (double) (tmaxW[0] -
                                                                 tminW[0]);
            for (i = 1; i <= pStart->table.nCol; i++) {
    ...
    if (HtmlTraceMask & HtmlTrace_Table5) {
        ...
        printf("nCol=%d minWidth=%d maxWidth=%d\n",
               pStart->table.nCol, pStart->table.minW[0],
               pStart->table.maxW[0]);
        for (i = 1; i <= pStart->table.nCol; i++) {
            TRACE_INDENT;
...
*/
static HtmlElement *
MinMax(htmlPtr, p, pMin, pMax, lineWidth, hasbg)
    HtmlWidget *htmlPtr;               /* The Html widget */
    ...
    int hasbg;
{
    ...
    int x1 = 0;                        /* Length of current line assuming
                                        * maximum length */
    int x2 = 0;                        /* Length of current line assuming
                                        * minimum length */
...
*/
HtmlElement *
HtmlTableLayout(pLC, pTable)
    HtmlLayoutContext *pLC;            /* The layout context */
    HtmlElement *pTable;               /* The <table> element */
{
    HtmlElement *pEnd;                 /* The </table> element */
    HtmlElement *p;                    /* For looping thru elements of the
                                        * table */
    HtmlElement *pNext;                /* Next element in the loop */
    HtmlElement *pCaption;             /* Start of the caption text.  The
                                        * <caption> */
    HtmlElement *pEndCaption;          /* End of the caption.  The </caption> 
                                        */
    int width;                         /* Width of the table as drawn */
    int cellSpacing;                   /* Value of cellspacing= parameter to
                                        * <table> */
    int cellPadding;                   /* Value of cellpadding= parameter to
                                        * <table> */
    int tbw;                           /* Width of the 3D border around the
                                        * whole table */
    int cbw;                           /* Width of the 3D border around a
                                        * cell */
    int pad;                           /* cellPadding + borderwidth */
    char *z;                           /* A string */
    int leftMargin;                    /* The left edge of space available
                                        * for drawing */
    int lineWidth;                     /* Total horizontal space available
                                        * for drawing */
    int specWidth;                     /* Total horizontal drawing width per
                                        * width= attr */
    int separation;                    /* Distance between content of columns 
                                        * (or rows) */
    int i;                             /* Loop counter */
    int n;                             /* Number of columns */
    ...
    int colspan;                       /* Number of columns spanned by
                                        * current cell */
    int vspace;                        /* Value of the vspace= parameter to
                                        * <table> */
    int hspace;                        /* Value of the hspace= parameter to
                                        * <table> */
    int rowBottom;                     /* Bottom edge of content in the
                                        * current row */
    int defaultVAlign;                 /* Default vertical alignment for the
                                        * current row */
    char *zAlign;                      /* Value of the ALIGN= attribute of
                                        * the <TABLE> */
#define N (HTML_MAX_COLUMNS+1)
    ...
    int ymax[N];                       /* Bottom edge of cell's content if
                                        * valign=top */
    HtmlElement *apElem[N];            /* The <td> or <th> for each cell in a 
                                        * row */
    ...
    /*
     * Figure how much horizontal space is available for rendering ** this
     * table.  Store the answer in lineWidth.  leftMargin is ** the left-most 
     * X coordinate of the table.  btm stores the top-most ** Y coordinate. 
     */
    ...
    /*
     * figure out how much space the table wants for each column, ** and in
     * total.. 
     */
    pEnd = TableDimensions(pLC->htmlPtr, pTable, lineWidth);

    /*
     * If we don't have enough horizontal space to accomodate the minimum
     * table ** width, then try to move down past some obstruction (such as
     * an ** <IMG ALIGN=LEFT>) to give us more room. 
     */
    if (lineWidth < pTable->table.minW[0]) {
        HtmlWidenLine(pLC, pTable->table.minW[0], &leftMargin, &btm,
                      &lineWidth);
    ...
    /*
     * Compute the width and left edge position of every column in ** the
     * table 
     */
    ...
    /*
     * Add notation to the pTable structure so that we will know where ** to
     * draw the outer box around the outside of the table. 
     */
    btm += vspace;
    pTable->table.y = btm;
    ...
    for (iRow = 1; iRow <= pTable->table.nRow; iRow++) {
         ...
         */
        for (iCol = 1; iCol <= pTable->table.nCol && iCol <= HTML_MAX_COLUMNS;
             iCol++) {
        ...
        for (p = p->pNext; p && p->base.type != Html_TR && p != pEnd; p = pNext) {
            ...
            switch (p->base.type) {
                ...
                case Html_TH:
                    /*
                     * Find the column number for this cell.  Be careful to
                     * skip ** columns which extend down to this row from
                     * prior rows 
                     */
                    do {
                        iCol++;
                    ...
                    TRACE(HtmlTrace_Table2,
                          ("Column %d: x=%d w=%d\n", iCol, x[iCol], w[iCol]));
                    /*
                     * Process the new cell.  (Cells beyond the maximum
                     * number of ** cells are simply ignored.) 
                     */
                    if (iCol <= HTML_MAX_COLUMNS) {
                        ...
                        if (HtmlUnlock(cellContext.htmlPtr))
                            return 0;
#ifdef TABLE_TRIM_BLANK
                        /*
                        ...
                        /*
                         * Advance the column counter for cells spaning
                         * multiple columns 
                         */
                        if (colspan > 1) {
                            iCol += colspan - 1;
                ...
                case Html_CAPTION:
                    /*
                     * Gotta remember where the caption is so we can render
                     * it ** at the end 
                     */
                    pCaption = p;
                    while (pNext && pNext != pEnd
                           && pNext->base.type != Html_EndCAPTION) {
                        pNext = pNext->pNext;
                    }
                    pEndCaption = pNext;
                    break;
         ...
         */
        for (iCol = 1; iCol <= pTable->table.nCol; iCol++) {
            int dy;                    /* Extra space at top of cell used for 
                                        * vertical alignment */
             ...
             */
            if (apElem[iCol] == 0
                || (iRow != pTable->table.nRow && lastRow[iCol] > iRow)) {
                continue;
            }
